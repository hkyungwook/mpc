...
CCircuit* CREATE_CIRCUIT(int nParties, const string& name, const vector<int>& params)
{
	...
	else if ( name == "btreetest" )
	{
		pCircuit = new CBTreeTest();
	}
...
void CCircuit::SaveFP(const char* filename)
{
	ofstream f(filename);
	...
	for(int i=nGateStart; i<nNumGates; i++)
	{
		...
		f << "gate," << i-2 << ",2 ";
		if( g->type == G_AND ) f << "0 0 0 1,";
		else f << "0 1 1 0,";
		f << g->left-2 <<"," << g->right-2 << endl;
	} 
	...
	for(int i=0; i<nNumParties; i++)
	{
		f << "output,a[" << i << "],output," << GetOutputStart(i)-2 << endl;
	}
