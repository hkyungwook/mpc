...
void CClient::RunOTThread()
{
	...
	ZZ q = CConfig::GetInstance()->GetOrder();
	if( q == 0 ) q = (p-1)/2;

	int FIELD_SIZE_IN_BYTES = NumBytes(p);
	int	FIELD_SIZE_IN_BITS = FIELD_SIZE_IN_BYTES * BITS_IN_BYTE;
	int ORDER_SIZE_IN_BYTES = NumBytes(q);
	int ORDER_SIZE_IN_BITS = ORDER_SIZE_IN_BYTES * BITS_IN_BYTE;
	...
	rnd.Create( NUM_EXECS_NAOR_PINKAS*2*ORDER_SIZE_IN_BITS, m_aSeed, m_nCounter);
	pBufIdx = rnd.GetArr();
	for(int i=0; i<NUM_EXECS_NAOR_PINKAS; i++)
	{
		ZZFromBytes(ztmp, pBufIdx, ORDER_SIZE_IN_BYTES);
		rem(pR0[i], ztmp, q);
		pBufIdx += ORDER_SIZE_IN_BYTES;
	...
	pBufIdx = pBuf2;
	sha1_context sha;
	BYTE* tmp = new BYTE[FIELD_SIZE_IN_BYTES];
	SHA_BUFFER	buf_key;
	for(int i=0; i<NUM_EXECS_NAOR_PINKAS; i++)
	{
		// put g^r0
