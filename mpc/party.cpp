...
BOOL CParty::Init()
{
	...
	m_g = CConfig::GetInstance()->GetGenerator();
	m_q = CConfig::GetInstance()->GetOrder();
	if( m_q == 0 ) 	m_q = (m_p-1)/2;
...
BOOL CParty::MainThreadRunSendInputShare()
{
	...
	int nSizeBytes = BITS_TO_BYTES(nSize);
	for( int i=0; i<m_nNumParties; i++ )
	{
		if( i != m_nPID )
		{
...
BOOL CParty::ThreadRunRecvInputShare(int nSndID)
{
	...
	int nSize = nEnd - nStart + 1;
	int nTotalBytes = BITS_TO_BYTES(nSize);
...
BOOL CParty::ThreadRunIKNPSndFirst(int nRcvID)
{
	...
	CBitVector rnd;
	rnd.Create(NUM_EXECS_NAOR_PINKAS*nFieldSize*BITS_OF_BYTE, m_aSeed, nRcvID, m_vCounter[nRcvID]);
	...
	int nToRcvBytes = BITS_TO_BYTES(NUM_EXECS_NAOR_PINKAS*4*m_nANDGates);
...
BOOL CParty::ThreadRunIKNPSndSecond(int nRcvID)
{
	...
	int nOTBytes = BITS_TO_BYTES(m_nANDGates*4);
	...
	while(i<m_nANDGates)
	{
		...
		sock.Send(v.GetArr(), BITS_TO_BYTES(j*4));
	}
...
BOOL CParty::ThreadRunIKNPRcvFirst(int nSndID)
{
	...
	// random k
	CBitVector rnd;
	rnd.Create(4*nFieldSize*BITS_OF_BYTE, m_aSeed, nSndID, m_vCounter[nSndID]);
	BYTE* pBufRnd = rnd.GetArr();
	...
	int nANDGatesBytes = BITS_TO_BYTES(m_nANDGates);

	CBitVector& T = m_vIKNPT[nSndID];
	
	T.Create(NUM_EXECS_NAOR_PINKAS * (m_nANDGates+BITS_OF_INT), m_aSeed, nSndID, m_vCounter[nSndID]);  
	...
	T0 =  (int*) T.GetArr();
	int nANDGateInts = BITS_TO_INTS(m_nANDGates);
	...
	while( i < m_nANDGates )
	{
		...
		for( j=0, w=SHA1_BITS; j<OT_WINDOW_SIZE && i < m_nANDGates; i++, j++, w++ )
		{
			...
			if(w == SHA1_BITS)
			{
				for(k=0; k<NUM_EXECS_NAOR_PINKAS; k++)
				{
					in = T0 + (k*nANDGateInts + i/BITS_OF_INT);
			...
			*(int*) P  = 0;
			P[input.Get2Bits(i)] = 1;

 			for(k=0; k<NUM_EXECS_NAOR_PINKAS; k++)
			{
				v.SetBit(t++, GETBIT((*pKeyMTx).buf[k][0].data, w) ^ P[0]);
				v.SetBit(t++, GETBIT((*pKeyMTx).buf[k][1].data, w) ^ P[1]);
				v.SetBit(t++, GETBIT((*pKeyMTx).buf[k][2].data, w) ^ P[2]);
				v.SetBit(t++, GETBIT((*pKeyMTx).buf[k][3].data, w) ^ P[3]);
			}
		}
		
		int nSize = BITS_TO_BYTES(j*NUM_EXECS_NAOR_PINKAS*4);
...
BOOL CParty::ThreadRunIKNPRcvSecond(int nSndID)
{
	...
	int nToRcvBytes = BITS_TO_BYTES(4*m_nANDGates);
	...
	while( i < m_nANDGates )
	{
		...
		int nANDGateInts = BITS_TO_INTS(m_nANDGates);
		int nANDGateRound = nANDGateInts*BITS_OF_INT;
		j=0;
		for(; j<OT_WINDOW_SIZE && i<m_nANDGates; i++, j++)
		{
			u = input.Get2Bits(i);
			...
			for(int k=0; k<NUM_EXECS_NAOR_PINKAS; k++)
			{
				tj.SetBit(k, T.GetBit(k*nANDGateRound+i));
			}
			sha1_update(sha, tj.GetArr(), NUM_NP_BYTES);
			sha1_finish(sha, sha_buf);
...
BOOL CParty::ThreadRunMultiplySender(int nRcvID)
{
	...
	int nRcvBytes = BITS_TO_BYTES(nSize*2);
	...
	*/
	...
	int nSndBytes = BITS_TO_BYTES(nSize*4);
...
BOOL CParty::ThreadRunMultiplyReceiver(int nSndID)
{
	...
	int nSndBytes = BITS_TO_BYTES(nSize*2);
	...
	*/
	...
	sock.Send(vSnd.GetArr(), nSndBytes);
	int nRcvBytes = BITS_TO_BYTES(nSize*4);
...
BOOL CParty::ThreadRunRecvOutputShare(int nSndID)
{
	...
	int o_end = m_pCircuit->GetOutputEnd(m_nPID);
	int o_size = o_end - o_start + 1;
	int nSizeBytes = BITS_TO_BYTES(o_size);
